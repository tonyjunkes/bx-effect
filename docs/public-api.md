...
| `models.effect.context.Context@bxEffect` | `empty`, `add`, `get`, `has`, `merge`. | Immutable service map. Missing tags are `BXEffect.MissingService` defects with deterministic diagnostics. |
